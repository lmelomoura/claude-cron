...
**`scope` (dev/runtime) foi implementado depois do bloco, e a cobertura da spec está completa.** Coluna `scope` no ledger pelo padrão aditivo do `_FINDING_COLUMNS` (`ALTER TABLE` guardado por `PRAGMA table_info`, como o `cwe`/`owasp`); escrita pelos **dois** produtores da categoria a partir de **uma só** função (`deps.merge_scope`), porque só um deles corre por análise e qual deles depende de a máquina ter o Trivy; visível nos três formatos de relatório e nas linhas que o ecrã lê. **Não é entrada do fingerprint** — o `rename_rule` recusa `dependency`, portanto uma mudança de identidade nesta categoria seria irrecuperável. O que cada formato consegue responder está medido, não presumido: `package-lock.json` (`"dev": true`), `composer.lock` (`packages-dev`) e `poetry.lock` (`category` até à lock-version 1.1, `groups` a partir da 2.1) respondem; `requirements.txt` e `go.sum` não conseguem e dizem `unknown`, nunca `runtime`. A única divergência entre os dois produtores — um `poetry.lock` de lock-version 2.0, onde o Trivy lê o `pyproject.toml` e o leitor manual não pode — está declarada em nota de cobertura nos dois lados, e é sempre no sentido seguro. A paridade foi corrida ao vivo com os dois produtores sobre a mesma árvore, e está fixada em `tests/security/test_adapters.py`.
