...
**A `sast` finding whose rationale starts "Semgrep's …" came from the pre-pass, and its identity is NOT built from a snippet.** Reuse the fingerprint `checklist` printed for it, exactly as printed. Recomputing one with `--snippet` mints a *second* identity for the same weakness: the pre-pass keeps re-finding the first every run while your re-report stays `new` for ever, and no decision anyone takes ever sticks to either of them.
