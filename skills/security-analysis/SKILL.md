...
- **Do not re-report a deterministic row the checklist shows as `open` or `partial` — not in this job.** `prepare` re-found it this run, the git-history sweep included, so re-reporting it here changes nothing but its text. Improving its text is Job 2's whole purpose, with a corrected severity and a rationale that says why; this job is only about the rows nothing re-found.
