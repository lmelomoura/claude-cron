...
# (CC_PORT_BASE, CC_PORT_SPAN wide). These turn that block into concrete numbers.
...
#
#     cc_port POSTGRES_PORT      -> 21003
#
# Outside a run (no CC_PORT_BASE) it echoes the fallback, so a hook can be tried
# by hand without pretending to be a run.
cc_port() { # <NAME> [fallback]
  local name="${1:?cc_port needs a name}" fallback="${2:-}"
  if [ -z "${CC_PORT_BASE:-}" ]; then printf '%s\n' "$fallback"; return 0; fi
  local span="${CC_PORT_SPAN:-100}" file="${TMPDIR:-/tmp}/cc-ports.$$.$CC_PORT_BASE"
  local seen n
  ...
  # first cc_port of every run would print a spurious "No such file" into
  # exec.log. Ask whether the file is there instead.
  n=0
  [ -f "$file" ] && n="$(wc -l < "$file" | tr -d ' ')"
  if [ "$n" -ge "$span" ]; then
    echo "cc_port: this run has used all $span ports of its block" >&2
    printf '%s\n' "$fallback"; return 1
  fi
  local port=$(( CC_PORT_BASE + n ))
  printf '%s %s\n' "$name" "$port" >> "$file"
  printf '%s\n' "$port"
}
...
cc_env_ports() { # <dotenv-file> [KEY_SUFFIX, default _PORT]
  local file="${1:?}" suffix="${2:-_PORT}" key
  [ -f "$file" ] || return 0
  while IFS= read -r key; do
    [ -n "$key" ] || continue
    cc_env_set "$file" "$key" "$(cc_port "$key")"
...
cc_copy_ignored() { # <path relative to the repo root> [...]
  local f
  for f in "$@"; do
    [ -f "$CC_REPO_PATH/$f" ] || continue
    mkdir -p "$(dirname "$f")"
    # -c asks APFS for a clone: same bytes, no copy, no extra disk.
    cp -c "$CC_REPO_PATH/$f" "$f" 2>/dev/null || cp "$CC_REPO_PATH/$f" "$f"
  done
}
